  ...
  const handleShare = () => {
    if (navigator.share) {
      ...
      .catch(() => {
        ...
        toast({
          title: language === 'th' ? 'คัดลอก URL แล้ว' 
            : language === 'en' ? 'URL copied to clipboard' 
            : language === 'cn' ? 'URL已复制到剪贴板' 
            : 'URLがクリップボードにコピーされました',
          description: language === 'th' ? 'ลิงก์ถูกคัดลอกไปยังคลิปบอร์ดแล้ว'
            : language === 'en' ? 'The link has been copied to your clipboard'
            : language === 'cn' ? '链接已复制到您的剪贴板'
          ...
          <div className="text-center">
            <h2 className="text-2xl font-bold mb-4">
              {language === 'th' ? 'ไม่พบกิจกรรม' 
                : language === 'en' ? 'Event Not Found'
                : language === 'cn' ? '未找到活动'
                : 'イベントが見つかりません'}
            </h2>
            <p className="text-gray-500 mb-6">
              {language === 'th' ? 'ไม่พบกิจกรรมที่คุณกำลังมองหา' 
                : language === 'en' ? 'The event you are looking for could not be found'
                : language === 'cn' ? '无法找到您正在寻找的活动'
                : 'お探しのイベントが見つかりませんでした'}
            </p>
            <Link to="/events">
              <Button>
                <ArrowLeft className="mr-2 h-4 w-4" />
                {language === 'th' ? 'กลับไปยังรายการกิจกรรม' 
                  : language === 'en' ? 'Back to Events'
                  : language === 'cn' ? '回到活动列表'
                  : 'イベント一覧に戻る'}
              </Button>
            </Link>
        ...
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-end">
          <div className="container pb-16 md:pb-20">
            <div className="max-w-3xl">
              <Badge 
                className={`${event.categoryColor} text-white mb-4`}
              >
            ...
            <Card>
              <CardContent className="p-6">
                <h2 className="text-2xl font-bold mb-6">
                  {language === 'th' ? 'รายละเอียดงาน' 
                    : language === 'en' ? 'Event Details'
                    : language === 'cn' ? '活动详情'
                    : 'イベント詳細'}
                </h2>
                ...
                <div className="prose max-w-none">
                  <p className="text-lg mb-8">
                    {event.description?.[language as keyof typeof event.description] || (
                      language === 'th' ? 'ไม่มีข้อมูลรายละเอียดเพิ่มเติม' 
                        : language === 'en' ? 'No additional information available'
                        : language === 'cn' ? '没有更多信息'
                        : '追加情報はありません'
                    )}
                  </p>
              ...
              <CardContent className="p-6">
                <h3 className="font-medium text-lg mb-4">
                  {language === 'th' ? 'ข้อมูลสำคัญ' 
                    : language === 'en' ? 'Essential Information'
                    : language === 'cn' ? '重要信息'
                    : '重要情報'}
                </h3>
                ...
                >
                  ...
                  {language === 'th' ? 'แชร์กิจกรรมนี้' 
                    : language === 'en' ? 'Share this event'
                    : language === 'cn' ? '分享此活动'
                    : 'このイベントをシェアする'}
                </Button>
              </CardContent>
